...
def meta_gradient_step(model,
    ...
    if order == 1:
        if train and len(task_losses) != 0:
            ...
            z_dummy = model.encode(torch.zeros(x.shape[0],x.shape[1]).float(), \
                    torch.zeros(train_pos_edge_index.shape[0],train_pos_edge_index.shape[1]).long(), fast_weights)
            loss = model.recon_loss(z_dummy,torch.zeros(train_pos_edge_index.shape[0],\
                    train_pos_edge_index.shape[1]).long())
            loss.backward()
            optimiser.step()
